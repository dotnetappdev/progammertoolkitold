...
namespace screenshareav.Database
{
    public static class DatabaseManager
    {
        ...
        public static List<(string Key, string Value)> GetDataByCategory(string category)
        {
            var list = new List<(string, string)>();
            using var conn = new SqliteConnection(ConnectionString);
            conn.Open();
            var cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT Key, Value FROM GeneralData WHERE Category = $category ORDER BY UpdatedAt DESC";
            cmd.Parameters.AddWithValue("$category", category);
            using var reader = cmd.ExecuteReader();
            while (reader.Read())
            {
